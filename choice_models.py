...
def choice_model_rank_based(recommendations: pd.DataFrame, alpha: float = 0.1, size = 5):
    """
    ...
    for user_id in tqdm(recommendations['user_id'].unique(), desc='Applying choice model'):
        ...
        chosen_rec = np.random.choice(range(0, len(recs)), size=size, p=p_list)
        for r in chosen_rec:
            acc_list.append([user_id, recs.iloc[r]['item_id']])
